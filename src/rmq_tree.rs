...
impl PackedArray {
    ...
    fn push(&mut self, value: i64) {
        ...
        debug_assert!(
            self.bits_per_element == 64 || value < (1i64 << self.bits_per_element),
            "value {value} does not fit in {} bits",
            self.bits_per_element
        );
        let bits = value as u64;
        for b in 0..self.bits_per_element {
            self.data.push((bits >> b) & 1 != 0);
        }
        self.len += 1;
    }
}
...
impl RmqTree {
    ...
    pub fn new(values: &[i64], block_bits: usize) -> Self {
        ...
        let x = PackedArray::with_values(values, bpe);
        ...
        let tree = PackedArray::with_values(&tree_vals, bpe);
    ...
    pub fn rmq(&self, mut i: usize, mut j: usize) -> i64 {
    ...
    pub fn psv(&self, mut i: usize, ub: i64) -> Option<usize> {
        // Linear scan nearby.
        let start = i;
    ...
    pub fn nsv(&self, mut i: usize, ub: i64) -> Option<usize> {
        // Linear scan nearby.
        let start = i;
...
mod tests {
    ...
    fn rmq_small() {
        ...
        let tree = RmqTree::new(&vals, 1); // block size = 2
        for i in 0..vals.len() {
            for j in i..vals.len() {
                assert_eq!(
                    tree.rmq(i, j),
    ...
    fn various_block_sizes() {
        ...
        for block_bits in 1..=5 {
            let tree = RmqTree::new(&vals, block_bits);
